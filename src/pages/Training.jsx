...
export default function Training() {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      {editing && <SessionModal session={editing} profile={profile} onClose={saved => { setEditing(null); if (saved) loadList() }} />}
    </div>
  )
}
...
function SessionDetail({ sessionId, activeUsers, profile, onBack }) {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-wrap items-start gap-3 mb-5">
        <div>
          ...
          <div className="flex flex-wrap gap-2 mt-3 text-sm text-gray-700">
            <Meta icon={Calendar}>{session.session_date ? formatDate(session.session_date) : '—'}</Meta>
      ...
      {editing && <SessionModal session={session} profile={profile} onClose={saved => { setEditing(false); if (saved) load() }} />}
...
function SessionModal({ session, profile, onClose }) {
  ...
  const save = async () => {
    ...
    if (isNew) {
      ...
      ;({ error } = await supabase.from('training_sessions').update(payload).eq('id', session.id))
    }
    setSaving(false)
    if (error) { alert(error.message.includes('duplicate') ? 'That signup link is already taken — choose another.' : error.message); return }
  ...
  return (
    <Modal title={isNew ? 'New training session' : 'Edit session'} subtitle={isNew ? 'Set up a session and its public signup link.' : 'Update the session details.'} onClose={() => onClose(false)}
      ...
      <div className="grid grid-cols-2 gap-3">
        <Field label="Date" req><input type="date" className={inputCls} value={f.session_date} onChange={e => set('session_date', e.target.value)} /></Field>
      ...
      <div className="grid grid-cols-2 gap-3">
        <Field label="Capacity"><input type="number" className={inputCls} value={f.capacity} onChange={e => set('capacity', e.target.value)} /></Field>
        <Field label="Fee (RM)"><input type="number" className={inputCls} value={f.fee} onChange={e => set('fee', e.target.value)} /></Field>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <Field label="Duration"><input className={inputCls} value={f.duration} onChange={e => set('duration', e.target.value)} placeholder="1 Day · 9am–5pm" /></Field>
        <Field label="Level"><input className={inputCls} value={f.level} onChange={e => set('level', e.target.value)} placeholder="Beginner – Intermediate" /></Field>
      </div>
